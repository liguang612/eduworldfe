...
const CourseCard: React.FC<CourseCardProps> = ({ imageUrl, title, details }) => (
  <div className="flex flex-col gap-3 pb-3 px-2">
    <div
      className="w-full bg-center bg-no-repeat aspect-square bg-cover rounded-xl"
      style={{ backgroundImage: `url("${imageUrl || 'https://via.placeholder.com/300x300?text=No+Image'}")` }}
    ></div>
    <div>
      <p className="text-[#0e141b] text-base font-medium leading-normal truncate" title={title}>{title}</p>
      <p className="text-[#4e7397] text-sm font-normal leading-normal">{details}</p>
    </div>
  </div>
);
...
const ExamItem: React.FC<ExamItemProps> = ({ title, course, dueDate }) => (
  <div className="flex items-center gap-4 bg-slate-50 px-4 min-h-[72px] py-2 justify-between">
    <div className="flex flex-col justify-center overflow-hidden mr-2">
      <p className="text-[#0e141b] text-base font-medium leading-normal line-clamp-1" title={title}>{title}</p>
      <p className="text-[#4e7397] text-sm font-normal leading-normal line-clamp-2" title={course}>{course}</p>
    </div>
    <div className="shrink-0">
      <p className="text-[#0e141b] text-base font-normal leading-normal">{dueDate}</p>
    </div>
  </div>
);
          ...
          <div className="px-4 sm:px-10 md:px-20 lg:px-40 flex flex-1 justify-center py-5">
            <div className="layout-content-container flex flex-col max-w-[960px] flex-1 w-full">
                  ...
                  >
                    <div className="flex flex-col gap-2 text-left">
                      ...
                      <h2 className="text-white text-xs sm:text-sm font-normal leading-normal @[480px]:text-base @[480px]:font-normal @[480px]:leading-normal">
                        We're glad you're here. Explore popular courses, lectures and more.
                      </h2>
                    </div>
                    <label className="flex flex-col min-w-40 h-12 sm:h-14 w-full max-w-[480px] @[480px]:h-16">
                      <div className="flex w-full flex-1 items-stretch rounded-xl h-full">
                        ...
                        <input
                          placeholder="Search for a course or lecture"
              ...
              <div className="flex justify-between items-center px-4 pt-5 pb-3">
                <h2 className="text-[#0e141b] text-xl sm:text-[22px] font-bold leading-tight tracking-[-0.015em]">Course highlights</h2>
                ...
                <Slider {...sliderSettings}>
                  {highlightCourses.map((course) => (
                    <CourseCard
                      ...
                      title={course.name}
                      details={`${course.chapters.length} chapters`}
                    />
                  ))}
                </Slider>
              </div>
              ...
              <div className="flex justify-between items-center px-4 pt-5 pb-3">
                <h2 className="text-[#0e141b] text-xl sm:text-[22px] font-bold leading-tight tracking-[-0.015em]">Upcoming exams</h2>
              ...
              <div className="px-0 sm:px-4 pb-4">
                {upcomingExams.map((exam) => (
                  <ExamItem
                    ...
                    dueDate={`${formatDateTime(exam.openTime)} - ${formatDateTime(exam.closeTime)}`}
                  />
                ))}
              </div>
            </div>
